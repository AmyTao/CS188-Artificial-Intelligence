...
class Linear(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        # print(inputs[1].shape[0])
        # print(inputs[1].shape[1])       
        # print('hi')
        grad_features = np.dot(gradient, inputs[1].T)
        grad_weights = np.dot(inputs[0].T, gradient)
...
class SquareLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        # print(inputs[1].size)
        # exit()
        a_grad = gradient * (inputs[0] - inputs[1]) / inputs[0].size
        b_grad = gradient * -(inputs[0] - inputs[1]) / inputs[1].size
        # print(gradient)
        # print(a_grad, b_grad)
...
class SoftmaxLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        "*** YOUR CODE HERE (Q5) ***"
        # log_prob = log_softmax(inputs[0])
        p_i = np.exp(log_probs)
        summation = np.sum(inputs[1]*p_i,axis=-1,keepdims=True)
        # grad_logits = (summation - inputs[1]) * gradient * 1/inputs[0].shape[0]
        grad_logits = (p_i - inputs[1]) * gradient * 1/inputs[0].shape[0]
        grad_labels = -log_probs*gradient * 1/inputs[1].shape[0]
