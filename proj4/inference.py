...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # print(X_RANGE)
    # print(Y_RANGE)
    variableDomainsDict[OBS0]=[ i for i in range(manhattanDistance((X_RANGE,Y_RANGE),(0,0))+MAX_NOISE-1)]
    variableDomainsDict[OBS1]=[ i for i in range(manhattanDistance((X_RANGE,Y_RANGE),(0,0))+MAX_NOISE-1)]
    # print(variables)
    # print(edges)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for key in self.beliefs.keys():
            self.beliefs[key] = self.beliefs[key]*self.getObservationProb(observation,pacmanPos,key,jailPos)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        tempDict = self.beliefs.copy()
        tempBeliefs = DiscreteDistribution()
        for newPos in self.allPositions:
            summation = 0
            for oldPos in tempDict.keys():
                #optimization
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                summation += newPosDist[newPos]*tempDict[oldPos]
            tempBeliefs[newPos] = summation
        self.beliefs = tempBeliefs

        # for pos in oldPos:
        #     print(pos,self.getPositionDistribution(gameState, pos))
        "*** END YOUR CODE HERE ***"
